export const ETH_GAS_STATION_URL = 'https://biz.token.im/api/ethgas'

export const ETH_ADDRESS = `0x${'0'.repeat(40)}`

export const APPROVE_METHOD = 'approve(address,uint256)'

export const APPROVE_GAS = 100000

export const TOKENLON_FILLORDER_GAS = 800000

export const HEX_OF_BALANCE_OF = '70a08231'

export const HEX_OF_GET_ALLOWANCE = 'dd62ed3e'

export const REQUEST_TIMEOUT = 10000

export const MAX_WEBSOCKET_RECONNECT_TRIED_TIMES = 100

export const WEBSOCKET_TRY_CONNECT_INTERVAL = 1000

export const CACHE_DATA_SECONDS = 5 * 60
